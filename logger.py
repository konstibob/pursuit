...
class ExperimentLogger:
    ...
    def start_evaluation_phase(self, eval_id, category="training"):
        eval_base_dir = os.path.join(self.run_dir, "evaluations")
        eval_dir = os.path.join(eval_base_dir, category)
        os.makedirs(eval_dir, exist_ok=True)
        
        if category == "final":
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            self.current_eval_csv = os.path.join(eval_dir, f"eval_{eval_id}_{timestamp}.csv")
        else:
